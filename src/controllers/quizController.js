...
const createQuiz = async (req, res) => {
  const { title, mode, questions, language } = req.body;
  ...
  const quizLanguage = language || "uz";
  ...
  try {
    ...
    try {
      const newQuizResult = await pool.query(
        "INSERT INTO quizzes (title, code, creator_id, mode, language) VALUES ($1, $2, $3, $4, $5) RETURNING id",
        [title, code, creator_id, quizMode, quizLanguage]
      );
      quizId = newQuizResult.rows[0].id;
    } catch (modeError) {
      console.log(
        "Mode/Language column not found, inserting without:",
        modeError.message
      );
      const newQuizResult = await pool.query(
...
const goLive = async (req, res) => {
  ...
  try {
    ...
    if (quizResult.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Quiz not found or you are not the creator." });
    }

    await pool.query("UPDATE quizzes SET status = $1 WHERE id = $2", [
      "live",
      quizId,
    ]);
...
const joinQuiz = async (req, res) => {
  ...
  if (!code || !username) {
    return res
      .status(400)
      .json({ message: "Please provide quiz code and username." });
  }

  try {
    const quizResult = await pool.query(
      "SELECT id, mode FROM quizzes WHERE code = $1 AND status = $2",
      [code, "live"]
    );
    ...
    if (quizResult.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Live quiz not found with this code." });
    }
...
const submitAnswer = async (req, res) => {
  ...
  try {
    const variantResult = await pool.query(
      "SELECT is_correct FROM variants WHERE id = $1",
      [variantId]
    );
...
const getQuizByCode = async (req, res) => {
  ...
  try {
    const quizResult = await pool.query(
      "SELECT id, title, code, status FROM quizzes WHERE code = $1",
      [code]
    );
...
const updateQuiz = async (req, res) => {
  ...
  try {
    ...
    if (title) {
      await pool.query("UPDATE quizzes SET title = $1 WHERE id = $2", [
        title,
        quizId,
      ]);
    }
...
const pauseQuiz = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      quizCheck.rows[0].creator_id !== creatorId
    ) {
      return res.status(403).json({ message: "Not authorized" });
    }

    await pool.query("UPDATE quizzes SET is_paused = TRUE WHERE id = $1", [
      quizId,
    ]);
...
const resumeQuiz = async (req, res) => {
  ...
  try {
    ...
    await pool.query("UPDATE quizzes SET is_paused = FALSE WHERE id = $1", [
      quizId,
    ]);

...
function addMediaInput(button) {
  ...
  const currentMediaType = mediaSection.querySelector(
...
function addVariant(button) {
  ...
  const variantNum =
    variantsContainer.querySelectorAll(".variantText").length + 1;
...
async function createQuiz(event) {
  ...
  for (const block of questionBlocks) {
    ...
    const mediaType = mediaSection.querySelector(
      'input[type="radio"]:checked'
    ).value;
    const mediaUrls = [];
  ...
  try {
    const response = await fetch(`${API_URL}/quizzes`, {
      ...
      body: JSON.stringify({ title, mode, language, questions }),
    });
...
async function editQuiz(quizId) {
  try {
    ...
    data.questions.forEach((question, qIndex) => {
      ...
      questionBlock.querySelector(".questionText").value =
      ...
      if (question.media_url) {
        ...
        if (mediaUrls.length > 0) {
          const mediaSection = questionBlock.querySelector(
      ...
      const variantsContainer = questionBlock.querySelector(
        ".variants-container"
      );
    ...
    const createBtn = document.querySelector(
      '#createQuizSection button[type="submit"]'
    );
...
function showQuestion() {
  ...
  mediaContainer.innerHTML =
    '<div id="mediaContainer" style="display: flex; gap: 15px; flex-wrap: wrap; justify-content: center; margin: 20px 0;"></div>';
  ...
  if (
    ...
    mediaUrls.forEach((url, index) => {
      ...
      if (isVideo) {
        if (isYouTube) {
          ...
          if (videoId) {
            ...
            iframe.style.cssText =
              "border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); margin: 10px;";
          ...
          video.style.cssText =
            "max-width: 600px; max-height: 450px; width: 100%; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); background: #000; display: block;";
          ...
          video.onerror = function (e) {
            ...
            videoWrapper.innerHTML = `<div style="padding: 30px; background: #ffebee; border-radius: 8px; color: #c62828; max-width: 500px; text-align: center;">
                            ...
                            <p style="font-size: 14px; margin-bottom: 10px;">Error code: ${
        ...
        img.style.cssText =
          "max-width: 600px; max-height: 450px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); display: block; margin: 10px;";
  ...
  question.variants.forEach((variant, index) => {
    ...
    variantDiv.innerHTML = `
            <span class="variant-letter">${String.fromCharCode(
              65 + index
            )}</span>
  ...
  controlButtonsDiv.style.cssText =
  ...
  skipBtn.style.cssText =
    "padding: 12px 30px; font-size: 16px; background: #6c757d; border: none; border-radius: 8px; color: white; cursor: pointer;";
  ...
  finishBtn.style.cssText =
    "padding: 12px 30px; font-size: 16px; background: #dc3545; border: none; border-radius: 8px; color: white; cursor: pointer;";
...
async function finishQuiz() {
  ...
  try {
    ...
    const response = await fetch(
      `${API_URL}/quizzes/${currentQuizId}/leaderboard`
    );
    const leaderboard = await response.json();
...
async function openPresentation(quizId, quizTitle, quizCode) {
  ...
  try {
    ...
    document.getElementById("presentationUrl").textContent =
...
async function loadParticipants() {
  ...
  try {
    const response = await fetch(
      `${API_URL}/quizzes/${window.currentPresentationQuizId}/participants`
    );
    const participants = await response.json();

    const namesDiv = document.getElementById("participantsNames");
    namesDiv.innerHTML = participants
      .map(
        (p) =>
...
async function startSyncQuiz() {
  ...
  socket.emit("admin-next-question", {
    code: window.currentPresentationCode,
    questionIndex: 0,
  });
...
async function pauseSyncQuiz() {
  ...
  try {
    await fetch(
      `${API_URL}/quizzes/${window.currentPresentationQuizId}/pause`,
      {
        method: "POST",
        headers: { "x-auth-token": token },
      }
...
async function resumeSyncQuiz() {
  ...
  try {
    await fetch(
      `${API_URL}/quizzes/${window.currentPresentationQuizId}/resume`,
      {
        method: "POST",
        headers: { "x-auth-token": token },
      }
...
async function nextSyncQuestion() {
  try {
    const response = await fetch(
      `${API_URL}/quizzes/${window.currentPresentationQuizId}/next`,
      {
        method: "POST",
        headers: { "x-auth-token": token },
      }
...
async function setupSynchronizedQuiz(quizId, username) {
  try {
    ...
    document.getElementById("questionContainer").innerHTML =
      '<div style="text-align:center; padding:50px; font-size:24px;">Kutib turing... Admin test boshlaydi</div>';
    ...
    socket.on("quiz-paused", () => {
      clearInterval(timerInterval);
      document.getElementById("questionContainer").innerHTML +=
        '<div style="color:orange; text-align:center; margin-top:20px; font-size:20px;">⏸️ PAUZA</div>';
    });
